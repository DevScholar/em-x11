...
static void redraw(void) {
  ...
  XDrawLine(
    display, window, gc, block_x + 50, block_y, block_x + 50, block_y + 100);
  XDrawLine(
    display, window, gc, block_x, block_y + 50, block_x + 100, block_y + 50);
  ...
  XDrawString(
    display, window, gc, 10, 260, status_text, (int)strlen(status_text));
...
int main(void) {
  ...
  window = XCreateSimpleWindow(
    display, root, 80, 60, 400, 300, 0, BlackPixel(display, 0), 0x003020UL);
  ...
  font_sans =
    XLoadQueryFont(display, "-*-helvetica-bold-r-normal-*-14-*-*-*-*-*-*-*");
  if (font_fixed)
    XSetFont(display, gc, font_fixed->fid);

...
extern void
__real_glMaterialfv(GLenum face, GLenum pname, const GLfloat* params);
 ...
 * by requestAnimationFrame, so the wasm suspends via JSPI here and
...
static Cmd* next_cmd(void) {
  if (!g_recording || g_recording_id == 0)
    return NULL;
  if (g_recording_id >= g_lists_size)
    return NULL;
...
GLuint glGenLists(GLsizei range) {
  ...
  if (!ensure_lists_capacity(first + range - 1))
    return 0;
...
void glNewList(GLuint list, GLenum mode) {
  ...
  if (list == 0 || list >= g_lists_size || !g_lists[list].in_use)
    return;
  ...
  for (size_t i = 0; i < L->count; i++) {
    ...
    switch (c->tag) {
      ...
      case CMD_END:
        ...
        break;
      case CMD_VERTEX3F:
        exec_vertex3f(c->v[0], c->v[1], c->v[2]);
        break;
      case CMD_NORMAL3F:
        exec_normal3f(c->v[0], c->v[1], c->v[2]);
        break;
      case CMD_SHADEMODEL:
        exec_shade_model(c->e1);
        break;
      case CMD_MATERIALFV:
...
void __wrap_glMaterialfv(GLenum face, GLenum pname, const GLfloat* params) {
  if (g_recording) {
    ...
    exec_material(face, pname, params);
}

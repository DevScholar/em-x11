...
- **`references/` is strictly for reference only.** No build script, fetcher, or CMakeLists may depend on or read from `references/`. Pre-generated files (makestrs output, etc.) must be checked into `native/` instead.

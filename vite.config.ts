 ...
 * em_x11_finalize_demo().  They have a CMakeLists.txt but NO user‑written
...
function serveBuildArtifactsRaw(): Plugin {
  return {
    name: 'em-x11-serve-build-artifacts-raw',
    configureServer(server) {
...
function serveLayer1GeneratedHtml(): Plugin {
  ...
  return {
    name: 'em-x11-serve-layer1-generated-html',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        if (!req.url) return next();
        const pathname = req.url.split('?')[0];
...
function copyBuildArtifacts(): Plugin {
  return {
    name: 'em-x11-copy-build-artifacts',
    apply: 'build',
    closeBundle() {
      ...
      if (!existsSync(src)) {
        // eslint-disable-next-line no-console
        console.warn(
          `[em-x11] build/artifacts/ not found; skipping dist copy. ` +
...
function writeLayer1BuildHtml(): Plugin {
  return {
    name: 'em-x11-write-layer1-build-html',
    apply: 'build',
    closeBundle() {

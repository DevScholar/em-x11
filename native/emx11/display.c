...
Display* emx11_get_display(void) { return &g_display; }
...
/* O(n) linear scan of the window table. At EMX11_MAX_WINDOWS=256, the
...
static XID emx11_resource_alloc(Display* dpy) { return emx11_next_xid(dpy); }
...
int XSync(Display* display, Bool discard) {
  ...
  XFlush(display);
  emx11_js_flush_roundtrip();
  if (discard) {
    XEvent sink;
    while (emx11_event_queue_size(display) > 0)
      emx11_event_queue_pop(display, &sink);
  }
  return 0;
}
...
emx11_close_display_proc
XESetCloseDisplay(Display* dpy, int extension, emx11_close_display_proc proc) {
  (void)dpy;
  (void)extension;
  (void)proc;
  return NULL;

...
#define EM_X11_MAX_QUARKS 8192

typedef struct {
  char* name;
} QuarkEntry;

static QuarkEntry g_quarks[EM_X11_MAX_QUARKS];
static XrmQuark g_next_quark = 1; /* 0 reserved for NULLQUARK */

static XrmQuark quark_find(const char* s) {
  if (!s || !*s)
    return NULLQUARK;
  for (XrmQuark q = 1; q < g_next_quark && q < EM_X11_MAX_QUARKS; q++) {
    if (strcmp(g_quarks[q].name, s) == 0)
      return q;
  }
  return NULLQUARK;
}
...
XrmQuark XrmStringToQuark(_Xconst char* s) {
  ...
  if (existing != NULLQUARK)
    return existing;
  if (g_next_quark >= EM_X11_MAX_QUARKS)
    return NULLQUARK;
...
XrmQuark XrmPermStringToQuark(_Xconst char* s) { return XrmStringToQuark(s); }

char* XrmQuarkToString(XrmQuark quark) {
  if (quark <= 0 || quark >= g_next_quark)
    return NULL;
...
XrmDatabase XrmGetDatabase(Display* dpy) { return dpy ? dpy->db : NULL; }
 ...
 * with '*'. */
static int parse_pattern(const char* spec,
                         int* n_out,
                         XrmQuark** quarks_out,
                         char** bindings_out) {
  ...
  while (*p) {
    ...
    if (*p == '*') {
      ...
      break; /* end of pattern (whitespace or ':') */
  }
...
static void
db_add(XrmDatabase db, int n, XrmQuark* quarks, char* bindings, char* value) {
  if (!db)
    return;
...
void XrmPutLineResource(XrmDatabase* db_p, _Xconst char* line) {
  ...
  while (spec_len > 0 && (p[spec_len - 1] == ' ' || p[spec_len - 1] == '\t'))
    spec_len--;
  char spec[512];
  if (spec_len <= 0 || spec_len >= (int)sizeof(spec))
    return;
  ...
  XrmQuark* qs = NULL;
  char* bs = NULL;
  if (parse_pattern(spec, &n, &qs, &bs) != 0)
    return;
...
void XrmPutResource(XrmDatabase* db,
                    _Xconst char* spec,
                    _Xconst char* type,
                    XrmValue* value) {
  (void)type; /* type is currently always "String"; we don't track it */
  if (!db || !spec || !value || !value->addr)
    return;
  if (!*db)
    *db = db_new();
...
void XrmPutStringResource(XrmDatabase* db,
                          _Xconst char* spec,
                          _Xconst char* value) {
  if (!db || !spec || !value)
    return;
...
Status
XrmCombineFileDatabase(_Xconst char* file, XrmDatabase* dst, Bool override) {
...
static Bool match_entry(int ei,
                        int qi,
                        int n_query,
                        int n_entry,
                        const XrmQuark* eq,
  ...
  if (b == '*') {
    ...
    int delta = 0;
    if (q == names[qi])
      delta = 2 + 2; /* name + tight bonus */
    else if (q == classes[qi])
      delta = 1 + 2; /* class + tight bonus */
    else
      return False;
    return match_entry(ei + 1,
                       qi + 1,
                       n_query,
                       n_entry,
                       eq,
...
Bool XrmQGetResource(XrmDatabase db,
  ...
  if (type_return)
    *type_return = NULLQUARK;
  if (value_return) {
    value_return->size = 0;
    value_return->addr = NULL;
  }
  if (!db || !quark_name || !quark_class)
    return False;

  int n_query = 0;
  while (quark_name[n_query] != NULLQUARK)
    n_query++;
  ...
  for (XrmEntry* e = db->head; e; e = e->next) {
    ...
    if (match_entry(0,
                    ...
                    0,
                    &s)) {
      if (s > best_score || (s == best_score && best && e->seq > best->seq)) {
        best = e;
        best_score = s;
      }
    }
  ...
  if (!best)
    return False;
  if (type_return)
    *type_return = XrmStringToQuark("String");
  if (value_return) {
...
/* -- SearchList API used by Xt's GetResources fast-path ------------------- *
 *
 * Xt stack-allocates an XrmHashTable[100] per widget initialisation, calls
 * XrmQGetSearchList(db, widget_name_path, widget_class_path, list, 100),
...
Bool XrmQGetSearchList(XrmDatabase db,
                       ...
                       int list_size) {
  if (!list_return || list_size < 2)
    return False;

  int n = 0;
  if (names)
    while (names[n] != NULLQUARK)
      n++;
  ...
  for (int i = 0; i < n; i++) {
    ...
    list_return[3 + n + i] =
      (XrmHashTable)(uintptr_t)(unsigned)(classes ? classes[i] : NULLQUARK);
  }
  list_return[3 + 2 * n] = NULL;
  return True;
}

Bool XrmQGetSearchResource(XrmSearchList list,
                           XrmName name,
                           XrmClass class_,
                           XrmRepresentation* type_return,
                           XrmValue* value_return) {
  ...
  if (!list || list[0] != EMX_SEARCH_MAGIC)
    return False;
...
Bool XrmGetResource(XrmDatabase db,
  ...
  int nn = 0;
  while (name_q[nn] != NULLQUARK && nn < 31)
    nn++;
  int nc = 0;
  while (class_q[nc] != NULLQUARK && nc < 31)
    nc++;
  while (nc < nn)
    class_q[nc++] = NULLQUARK;
  class_q[nn] = NULLQUARK;

  XrmRepresentation type;
  Bool ok = XrmQGetResource(db, name_q, class_q, &type, value_return);
  if (ok && str_type_return)
    *str_type_return = XrmQuarkToString(type);
  return ok;
}

/* -- Command-line parsing (Xt -name -xrm etc.) ---------------------------- */

void XrmParseCommand(XrmDatabase* db,
...
void XrmStringToBindingQuarkList(_Xconst char* s,
...
void XrmQPutResource(XrmDatabase* db,
                     XrmBindingList bindings,
                     XrmQuarkList quarks,
                     XrmRepresentation type,
                     XrmValue* value) {

...
Status XkbAllocClientMap(XkbDescPtr xkb, unsigned int which, unsigned int nTypes) {
    (void)which;
    (void)nTypes;
    if (!xkb) return BadValue;
    if (xkb->map) return Success;             /* already allocated */
    XkbClientMapPtr m = (XkbClientMapPtr)calloc(1, sizeof(XkbClientMapRec));
    if (!m) return BadAlloc;
...
void XkbFreeClientMap(XkbDescPtr xkb, unsigned int what, Bool freeMap) {
    (void)what;
    if (!xkb || !xkb->map) return;
    XkbClientMapPtr m = xkb->map;
    if (m->syms)        { free(m->syms);        m->syms = NULL; }
    if (m->key_sym_map) { free(m->key_sym_map); m->key_sym_map = NULL; }
    if (m->modmap)      { free(m->modmap);      m->modmap = NULL; }
    if (m->types) {
        for (int i = 0; i < m->num_types; i++) {
            if (m->types[i].map) free(m->types[i].map);
            if (m->types[i].preserve) free(m->types[i].preserve);
            if (m->types[i].level_names) free(m->types[i].level_names);
        }
        free(m->types);
        m->types = NULL;
    }
...
static Status fill_client_map(Display *dpy, XkbDescPtr xkb) {
    ...
    for (int kc = xkb->min_key_code; kc <= xkb->max_key_code; kc++) {
        if (dpy->keysym_table[kc] != NoSymbol) active++;
    }
    int total_syms = active * 2;               /* 2 levels per active key */
    if (m->syms) free(m->syms);
    m->syms = (KeySym *)calloc((size_t)(total_syms > 0 ? total_syms : 1),
                                sizeof(KeySym));
    if (!m->syms) return BadAlloc;

 ...
 * em_x11_js_draw_string bridge. Glyph indices are kept as 1:1 with
...
static int family_is_css_generic(const char* s) {
  return strcmp(s, "sans-serif") == 0 || strcmp(s, "serif") == 0 ||
         strcmp(s, "monospace") == 0 || strcmp(s, "cursive") == 0 ||
         strcmp(s, "fantasy") == 0 || strcmp(s, "system-ui") == 0 ||
         strcmp(s, "ui-sans-serif") == 0 || strcmp(s, "ui-serif") == 0 ||
         strcmp(s, "ui-monospace") == 0 || strcmp(s, "math") == 0 ||
         strcmp(s, "emoji") == 0;
}
...
static const char* vendor_to_generic(const char* name) {
  ...
  if (strstr(buf, "times") || strstr(buf, "serif") || strstr(buf, "roman") ||
      strstr(buf, "charter") || strstr(buf, "georgia") ||
      strstr(buf, "cambria") || strstr(buf, "garamond") ||
      strstr(buf, "palatino")) {
    return "serif";
  }
  return "sans-serif";
...
static void
append_family(char* out, size_t outlen, size_t* cursor, const char* name) {
  if (!name || !*name)
    return;
  size_t pos = *cursor;
  if (pos >= outlen)
   ...
   * the trailing generic don't all stack up. */
  {
    ...
    for (size_t i = 0; i < nl; i++)
      tmp[i] = (char)tolower((unsigned char)name[i]);
    tmp[nl] = '\0';
    char hay[256];
    size_t hl = pos < sizeof(hay) ? pos : sizeof(hay) - 1;
    for (size_t i = 0; i < hl; i++)
      hay[i] = (char)tolower((unsigned char)out[i]);
    hay[hl] = '\0';
    if (strstr(hay, tmp))
      return;
  }
  ...
  if (generic) {
    written =
      snprintf(out + pos, outlen - pos, "%s%s", need_comma ? ", " : "", name);
  } else {
    /* Quote vendor names. We assume `name` doesn't contain a literal
     * `"`; X font names never do. */
    written = snprintf(
      out + pos, outlen - pos, "%s\"%s\"", need_comma ? ", " : "", name);
  }
  if (written < 0)
    return;
...
static void build_family_list(char* out, size_t outlen, FcPattern* pat) {
  ...
  for (n = 0;; n++) {
    FcChar8* fam = NULL;
    if (FcPatternGetString(pat, FC_FAMILY, n, &fam) != FcResultMatch)
      break;
    if (!fam || !*fam)
      continue;
    if (n == 0)
...
static int pixel_size_from_pattern(FcPattern* pat) {
  ...
  if (FcPatternGetInteger(pat, FC_PIXEL_SIZE, 0, &iv) == FcResultMatch &&
      iv > 0)
    return iv;
...
static void build_css_from_pattern(char* out,
                                   size_t outlen,
                                   FcPattern* pat,
                                   int* pixel_out) {
...
XftFont* XftFontOpenPattern(Display* dpy, FcPattern* pattern) {
  ...
  int widths[95] = {0};
  em_x11_js_measure_font(f->css, &ascent, &descent, &max_w, widths);
  f->ascent = ascent;
  f->descent = descent;
   ...
   * recover it. */
  int pixel_check;
  if (FcPatternGetInteger(pattern, FC_PIXEL_SIZE, 0, &pixel_check) !=
      FcResultMatch) {
    double pxd;
...
XftFont* XftFontOpen(Display* dpy, int screen, ...) {
  ...
  while (1) {
    ...
    switch (type) {
      case FcTypeInteger:
        FcPatternAddInteger(p, object, va_arg(ap, int));
        break;
      case FcTypeDouble:
        FcPatternAddDouble(p, object, va_arg(ap, double));
        break;
      case FcTypeString:
        FcPatternAddString(p, object, va_arg(ap, const FcChar8*));
        break;
      case FcTypeBool:
        FcPatternAddBool(p, object, va_arg(ap, int));
        break;
      case FcTypeMatrix:
        FcPatternAddMatrix(p, object, va_arg(ap, const FcMatrix*));
        break;
      case FcTypeCharSet:
        FcPatternAddCharSet(p, object, va_arg(ap, const FcCharSet*));
        break;
      default:
        va_arg(ap, void*);
        break;
...
FcBool XftNameUnparse(FcPattern* pat, char* dest, int len) {
  ...
  snprintf(dest,
           len,
           "%s:pixelsize=%d",
           family ? (const char*)family : "sans-serif",
           pixel);
  return FcTrue;
}
...
static void
extents_from_string(XftFont* font, const char* text, int len, XGlyphInfo* out) {
  int width =
    (text && len > 0) ? em_x11_js_measure_string(font->css, text, len) : 0;
...
FcBool XftDrawSetClipRectangles(
...
void XftDrawGlyphFontSpec(XftDraw* draw,
                          _Xconst XftColor* color,
                          _Xconst XftGlyphFontSpec* specs,
                          int nspec) {
  if (!draw || !specs || nspec <= 0)
    return;
...
void XftDrawRect(XftDraw* draw,
  ...
  if (!draw || !color)
    return;
  em_x11_js_fill_rect(
    (Window)draw->drawable, x, y, w, h, (unsigned long)color->pixel);
}

/* -- color -------------------------------------------------------------- */

FcBool XftColorAllocValue(Display* dpy,
                          Visual* visual,
                          Colormap cmap,
...
FcBool XftColorAllocName(Display* dpy,

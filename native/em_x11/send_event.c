...
Status XSendEvent(Display* display,
  ...
  if (em_x11_selection_intercept_send(display, w, event_send)) {
    return 1;
  }
  ...
  if (event_mask == 0) {
    return em_x11_event_queue_push(display, &ev) ? 1 : 0;
  }
  ...
  while (target) {
    ...
    if (!propagate)
      break;
    if (target->parent == None || target->parent == target->id)
      break;
    if (target->id == root)
      break;
    target = em_x11_window_find(display, target->parent);
  }
...
static void
push_focus_change(Display* dpy, int type, Window w, int mode, int detail) {
...
int XSetInputFocus(Display* display, Window focus, int revert_to, Time time) {
  ...
  display->focus_last_time =
    (time == CurrentTime) ? display->focus_last_time : time;

  if (old_focus != focus) {
    push_focus_change(
      display, FocusOut, old_focus, NotifyNormal, NotifyNonlinear);
    push_focus_change(display, FocusIn, focus, NotifyNormal, NotifyNonlinear);
  }
  ...
  em_x11_js_set_input_focus(focus);
  return 1;
}
...
int XGetInputFocus(Display* display,
  ...
  if (!display)
    return 0;
  if (focus_return)
    *focus_return = display->focus_window;
  if (revert_to_return)
    *revert_to_return = display->focus_revert_to;
  return 1;
}

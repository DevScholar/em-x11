...
static void elt_free(FcElt* e) {
  if (!e)
    return;
  free(e->object);
  if (e->val.type == FcTypeString)
    free(e->val.u.s);
  else if (e->val.type == FcTypeMatrix)
    free(e->val.u.m);
  else if (e->val.type == FcTypeCharSet)
    FcCharSetDestroy(e->val.u.c);
  free(e);
}
...
FcBool
FcPatternAddCharSet(FcPattern* p, const char* object, const FcCharSet* cs) {
...
FcResult
FcPatternGetInteger(const FcPattern* p, const char* o, int n, int* out) {
  FcElt* e = pattern_find_n(p, o, n);
  if (!e)
    return FcResultNoMatch;
  if (e->val.type == FcTypeInteger) {
    if (out)
      *out = e->val.u.i;
    return FcResultMatch;
  }
  if (e->val.type == FcTypeDouble) {
    if (out)
      *out = (int)e->val.u.d;
    return FcResultMatch;
  }
...
FcResult
FcPatternGetDouble(const FcPattern* p, const char* o, int n, double* out) {
  FcElt* e = pattern_find_n(p, o, n);
  if (!e)
    return FcResultNoMatch;
  if (e->val.type == FcTypeDouble) {
...
FcResult
FcPatternGetString(const FcPattern* p, const char* o, int n, FcChar8** out) {
  FcElt* e = pattern_find_n(p, o, n);
  if (!e)
    return FcResultNoMatch;
  if (e->val.type != FcTypeString)
...
FcResult
FcPatternGetBool(const FcPattern* p, const char* o, int n, FcBool* out) {
  FcElt* e = pattern_find_n(p, o, n);
  if (!e)
    return FcResultNoMatch;
  if (e->val.type != FcTypeBool)
    return FcResultTypeMismatch;
  if (out)
    *out = e->val.u.b;
  return FcResultMatch;
...
FcResult
FcPatternGetMatrix(const FcPattern* p, const char* o, int n, FcMatrix** out) {
  FcElt* e = pattern_find_n(p, o, n);
  if (!e)
    return FcResultNoMatch;
  if (e->val.type != FcTypeMatrix)
    return FcResultTypeMismatch;
  if (out)
    *out = e->val.u.m;
  return FcResultMatch;
...
FcPattern* FcPatternDuplicate(const FcPattern* src) {
  ...
  for (FcElt* e = src->elts; e; e = e->next) {
    switch (e->val.type) {
      case FcTypeInteger:
        FcPatternAddInteger(dst, e->object, e->val.u.i);
        break;
      case FcTypeDouble:
        FcPatternAddDouble(dst, e->object, e->val.u.d);
        break;
      case FcTypeString:
        FcPatternAddString(dst, e->object, e->val.u.s);
        break;
      case FcTypeBool:
        FcPatternAddBool(dst, e->object, e->val.u.b);
        break;
      case FcTypeMatrix:
        FcPatternAddMatrix(dst, e->object, e->val.u.m);
        break;
      case FcTypeCharSet:
        FcPatternAddCharSet(dst, e->object, e->val.u.c);
        break;
      default:
        break;
    }
...
static void apply_kv(FcPattern* p, char* key, char* val) {
  ...
  if (val) {
    if (is_numeric(val)) {
      if (strchr(val, '.'))
        FcPatternAddDouble(p, key, atof(val));
      else
        FcPatternAddInteger(p, key, atoi(val));
    } else if (strcmp(val, "true") == 0)
      FcPatternAddBool(p, key, FcTrue);
    else if (strcmp(val, "false") == 0)
      FcPatternAddBool(p, key, FcFalse);
    else
      FcPatternAddString(p, key, (const FcChar8*)val);
  } else {
...
FcPattern* FcNameParse(const FcChar8* name) {
  ...
  while (fam && *fam) {
    ...
    char* t = trim(fam);
    if (*t)
      FcPatternAddString(p, FC_FAMILY, (const FcChar8*)t);
...
FcBool FcFontSetAdd(FcFontSet* s, FcPattern* font) {
  ...
  if (s->nfont >= s->sfont) {
    ...
    FcPattern** np =
      (FcPattern**)realloc(s->fonts, nsfont * sizeof(FcPattern*));
    if (!np)
      return FcFalse;
    s->fonts = np;
    s->sfont = nsfont;
...
void FcDefaultSubstitute(FcPattern* p) {
  ...
  if (FcPatternGetInteger(p, FC_PIXEL_SIZE, 0, &pixel_int) != FcResultMatch) {
    double size;
    int size_int;
    if (FcPatternGetDouble(p, FC_PIXEL_SIZE, 0, &size) == FcResultMatch &&
        size > 0) {
      /* already pixels */
    } else if (FcPatternGetDouble(p, FC_SIZE, 0, &size) == FcResultMatch &&
               size > 0) {
      FcPatternAddDouble(p, FC_PIXEL_SIZE, size * 96.0 / 72.0);
    } else if (FcPatternGetInteger(p, FC_SIZE, 0, &size_int) == FcResultMatch &&
               size_int > 0) {
...
FcFontSet* FcFontSort(FcConfig* cfg,
                      FcPattern* p,
                      FcBool trim_,
                      FcCharSet** csp,
                      FcResult* result) {

...
#define PREDEFINED_COUNT                                                       \
  (sizeof(PREDEFINED_ATOMS) / sizeof(PREDEFINED_ATOMS[0]))
...
static Atom find_predefined(const char* name) {
  for (Atom i = 1; i < PREDEFINED_COUNT; i++) {
    if (strcmp(PREDEFINED_ATOMS[i], name) == 0)
      return i;
  }
  return None;
}
...
Atom XInternAtom(Display* dpy, _Xconst char* name, Bool only_if_exists) {
  ...
  if (pre != None)
    return pre;
  return em_x11_js_intern_atom(name, only_if_exists);
}
...
char* XGetAtomName(Display* dpy, Atom atom) {
  ...
  return em_x11_js_get_atom_name(atom);
}

Status
XGetAtomNames(Display* dpy, Atom* atoms, int count, char** names_return) {
  if (!atoms || !names_return || count <= 0)
    return 0;
  Status ok = 1;
  for (int i = 0; i < count; i++) {

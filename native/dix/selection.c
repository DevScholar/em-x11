 ...
 *         navigator.clipboard.readText() (via JSPI), encodes per
...
#define SEL_TABLE_SIZE                                                         \
  (sizeof(((Display*)0)->selections) / sizeof(((Display*)0)->selections[0]))
...
static int sel_find(Display* dpy, Atom sel) {
  for (int i = 0; i < (int)SEL_TABLE_SIZE; i++) {
    if (dpy->selections[i].sel == sel && sel != 0)
      return i;
  }
  return -1;
}
 ...
 * must free *out_bytes. */
static int
strip_crlf(const unsigned char* in, int in_len, unsigned char** out_bytes) {
...
static int utf8_to_latin1(const unsigned char* text,
                          int text_len,
...
Window XGetSelectionOwner(Display* dpy, Atom selection) {
  ...
  int slot = sel_find(dpy, selection);
  if (slot >= 0)
    return dpy->selections[slot].owner;

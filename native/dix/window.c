...
static void push_map_notify(Display* dpy, EmX11Window* win, bool mapped) {
  if (!wants_structure(dpy, win->id))
    return;
  XEvent ev = {0};
  if (mapped) {
    ev.type = MapNotify;
...
static void push_configure_notify(Display* dpy, EmX11Window* win) {
  XEvent ev = {0};
...
Window XCreateSimpleWindow(Display* display,
  ...
  EmX11Window* w = em_x11_window_alloc(display);
  if (!w) {
    return None;
  }
...
Window XCreateWindow(Display* display,
  ...
  if (attrs)
    XChangeWindowAttributes(display, w->id, valuemask, attrs);
  return w->id;
}
...
int XMapWindow(Display* display, Window w) {
  ...
  if (win) {
    ...
    if (state_changed) {
      display
        ->request++; /* match Tk's NextRequest() — see notify_js_reconfigure */
    ...
    if (state_changed && win->parent == display->screens[0].root &&
...
static void notify_js_reconfigure(Display* dpy, EmX11Window* win) {
 ...
 * local shadow for `w`, so we can't update an EmX11Window nor push a
 ...
 * the Host. */
static int notify_js_reconfigure_xconn(Display* dpy,
                                       Window w,
...
int XMoveResizeWindow(Display* display,
  ...
  if (!win) {
    ...
    v.width = (int)width;
    v.height = (int)height;
    return notify_js_reconfigure_xconn(
      display, w, CWX | CWY | CWWidth | CWHeight, &v);
  }
  win->x = x;
  win->y = y;
  win->width = width;
...
int XConfigureWindow(Display* display,
  ...
  if (valuemask & CWY)
    win->y = values->y;
  if (valuemask & CWWidth)
    win->width = (unsigned int)values->width;
  if (valuemask & CWHeight)
    win->height = (unsigned int)values->height;
...
Status XGetGeometry(Display* display,
  ...
  EmX11Window* win = em_x11_window_find(display, (Window)d);
  if (win) {
    if (root_return)
    ...
    if (y_return)
      *y_return = win->y;
    if (width_return)
      *width_return = win->width;
    if (height_return)
      *height_return = win->height;
    if (border_width_return)
      *border_width_return = win->border_width;
    if (depth_return)
...
Bool XTranslateCoordinates(Display* display,
                           ...
                           int* dest_y_return,
                           Window* child_return) {
  EmX11Window* src = em_x11_window_find(display, src_w);
  EmX11Window* dst = em_x11_window_find(display, dest_w);
  if (!src || !dst)
    return False;
  ...
  for (EmX11Window* cur = src; cur;) {
    src_ax += cur->x;
    src_ay += cur->y;
    if (cur->parent == None || cur->parent == cur->id)
      break;
    EmX11Window* p = em_x11_window_find(display, cur->parent);
    if (!p) {
  ...
  if (dest_x_return)
    *dest_x_return = src_ax + src_x - dst_ax;
  if (dest_y_return)
    *dest_y_return = src_ay + src_y - dst_ay;
  if (child_return)
    *child_return = None;
  return True;
}
...
Status XQueryTree(Display* display,
  ...
  for (int i = 0; i < got; i++)
    kids[i] = (Window)(unsigned int)buf[i];
  free(buf);
  if (children_return)
    *children_return = kids;
  if (nchildren_return)
    *nchildren_return = (unsigned int)got;
  return 1;
}
 ...
 * and twm wouldn't have a local EmX11Window for it. */
...
int XCirculateSubwindowsDown(Display* dpy, Window w) {
  (void)dpy;
  (void)w;
  return 1;
}

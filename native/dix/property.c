...
int XChangeProperty(Display* display,
  ...
  if (display && display->clipboard_proxy_win != None &&
      w == display->clipboard_proxy_win && display->incr_active) {
    if (em_x11_incr_handle_chunk(
          display, property, (const unsigned char*)data, nelements, format)) {
      return 1; /* consumed; do not store in JS property table */
    }
  }
  int rc =
    em_x11_js_change_property(w, property, type, format, mode, data, nelements);
...
int XGetWindowProperty(Display* display,
  ...
  int meta[EM_X11_PROP_META_SIZE] = {0};
  em_x11_js_get_property_meta(
    w, property, req_type, long_offset, long_length, meta);
  if (!meta[EM_X11_PROP_META_PRESENT])
    return BadWindow;

  if (actual_type_return)
    *actual_type_return = (Atom)meta[EM_X11_PROP_META_TYPE];
  if (actual_format_return)
    *actual_format_return = meta[EM_X11_PROP_META_FORMAT];
  if (nitems_return)
    *nitems_return = (unsigned long)meta[EM_X11_PROP_META_NITEMS];
  if (bytes_after_return)
    *bytes_after_return = (unsigned long)meta[EM_X11_PROP_META_BYTES_AFTER];
  ...
  if (!meta[EM_X11_PROP_META_FOUND] || meta[EM_X11_PROP_META_DATA_LEN] == 0) {
    return Success;
  }
  ...
  size_t data_bytes = (size_t)meta[EM_X11_PROP_META_DATA_LEN];
  unsigned char* buf = malloc(data_bytes + 1);
  if (!buf)
    return BadAlloc;
  em_x11_js_get_property_data(w,
                              property,
                              req_type,
                              long_offset,
                              long_length,
...
Status
XGetCommand(Display* display, Window w, char*** argv_return, int* argc_return) {
  (void)display;
  (void)w;
  if (argc_return)
    *argc_return = 0;
...
Atom* XListProperties(Display* display, Window w, int* num_prop_return) {
  ...
  if (num_prop_return)
    *num_prop_return = 0;
  int n = em_x11_js_list_properties_count(w);
  if (n <= 0)
    return NULL;
  Atom* list = malloc(sizeof(Atom) * (size_t)n);
  if (!list)
    return NULL;
  int got = em_x11_js_list_properties_fetch(w, list, n);
  if (num_prop_return)
    *num_prop_return = got;
  return list;
}
